...
function getUpdatedStatisticsForCurrentDay(oPreviousSongs, oPreviousStats, sDateLabel) {
    // returns updated json statistics
    if (!oPreviousSongs || 
        !oPreviousStats || 
        Object.keys(oPreviousSongs).length === 0) {
    ...
    Object.keys(oPreviousSongs).map(sMoment => oPreviousSongs[sMoment].number)
        .forEach(sNumber => {
...
function getUpdatedStatistics(aSortedSongsSung, aDateLabels) {
    ...
    aDateLabels.unshift("never"); // for the songs before the first song set

    aSortedSongsSung
        .map(oSongWithStats => removeStatsFromObject(oSongWithStats))
        .reduce((oPrevStats, oCurrSongs) => {
            // start from previous statistics
            var oNewStats = getUpdatedStatisticsForCurrentDay(oPrevSongs, oPrevStats, aDateLabels.shift());

            aStats.push(oNewStats);
            oPrevSongs = oCurrSongs;
