...
function getReadings (sYear, sFeastName, bLongReadingNames, bAddNotes) {
    ...
    if (bAddNotes) {
        return bLongReadingNames
            ? expandReadingsAbbreviations(oReadingData)
            : JSON.parse(JSON.stringify(oReadingData));
    }

    var oReadingInfo = Object.keys(oReadingData).reduce(function (o, sKey) {
        o[sKey] = oReadingData[sKey].map(oReading => oReading.reading);
        return o;
    }, {});

    return bLongReadingNames
        ? expandReadingsAbbreviations(oReadingInfo)
        : oReadingInfo;
}

function expandReadingsAbbreviations (oReadingData) {
    return Object.keys(oReadingData).reduce((o, sKey) => {
        ...
        var aMappedReadings = aReadings.map(function (vReading) {
            ...
            var sReadingMapped = sReadingLong
                ? sReading.replace(sReadingShort, sReadingLong)
                : sReading;
